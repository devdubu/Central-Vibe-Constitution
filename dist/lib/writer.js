...
export function getOutputPath(scopeResult) {
 ...
 */
export async function writeConstitution(content, scopeResult, confirm) {
    const outputPath = getOutputPath(scopeResult);
    if (existsSync(outputPath)) {
        const existing = await readFile(outputPath, 'utf-8');
        if (existing !== content) {
            const ok = await confirm(`${outputPath} already exists and differs from remote. Overwrite?`);
            if (!ok)
                return false;
        }
        else {
