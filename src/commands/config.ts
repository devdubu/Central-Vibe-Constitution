...
export async function runConfigSet(options: ConfigSetOptions): Promise<void> {
  ...
  if (options.token) {
    if (scopeResult.scope === 'project') {
      console.warn(
        'Warning: token is not stored in .cvcrc (git-committed). Use the CVC_TOKEN env var instead.',
      );
    } else {
  ...
  if (options.url) console.log(`  remote: ${updates['remote']}`);
  if (options.token && scopeResult.scope === 'global') console.log(`  token:  [set]`);
}
